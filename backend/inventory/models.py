...
class ItemCodeBidang(models.Model):
    golongan = models.ForeignKey(
        ...
        related_name='bidang_set'
    )
    code = models.CharField(_('kode bidang'), max_length=10)
...
class ItemCodeKelompok(models.Model):
    ...
    code = models.CharField(_('kode kelompok'), max_length=10)
...
class ItemCodeSubKelompok(models.Model):
    kelompok = models.ForeignKey(
        ItemCodeKelompok,
        on_delete=models.CASCADE,
        related_name='subkelompok_set'
    )
    code = models.CharField(_('kode sub kelompok'), max_length=15)
    base_description = models.TextField(_('uraian sub kelompok (dasar)'))
    ...
    class Meta:
        ...
        ordering = [
            'kelompok__bidang__golongan__code',
            'kelompok__bidang__code',
            'kelompok__code',
            'code'
        ]
    ...
    def get_base_code_prefix(self):
        ...
        try:
            ...
            bid = kel.bidang
            gol = bid.golongan
            return f"{gol.code}.{bid.code}.{kel.code}.{self.code}"
        except Exception:
             return "?.?.?.?"
    ...
    def get_full_base_code_prefix(self):
        ...
        try:
            ...
            bid = kel.bidang
            gol = bid.golongan
            gol_code = str(gol.code).zfill(1)
            bid_code = str(bid.code).zfill(2)
            kel_code = str(kel.code).zfill(2)
            skel_code = str(self.code).zfill(2)
...
class ItemCodeBarang(models.Model):
     ...
     sub_kelompok = models.ForeignKey(
         ItemCodeSubKelompok,
         on_delete=models.CASCADE,
         related_name='barang_set'
     )
     code = models.CharField(_('kode barang (akhir)'), max_length=3)
     base_description = models.TextField(_('uraian barang (dasar)'))
     account_code = models.CharField(
         _('kode akun'), max_length=20, blank=True, null=True
     )
     account_description = models.CharField(
         _('uraian akun'), max_length=100, blank=True, null=True
     )
     ...
     def _generate_full_base_code(self):
         ...
         if not self.sub_kelompok_id or not self.code:
             return None
         try:
             sk = ItemCodeSubKelompok.objects.select_related(
                 'kelompok__bidang__golongan'
             ).get(pk=self.sub_kelompok_id)
             prefix = sk.get_full_base_code_prefix()
             if prefix is None: return None
             brg_code = str(self.code).zfill(3)
         ...
         if generate_code:
             ...
             if generated_code:
                 ...
                 raise IntegrityError(
                     f"Tidak dapat men-generate full_base_code yang valid untuk "
                     f"ItemCodeBarang dengan subkel={self.sub_kelompok_id}, code={self.code}. "
                     f"Periksa data hierarki induknya."
                 )
         super().save(*args, **kwargs)

# --- MODEL VARIAN PRODUK SPESIFIK (DITAMBAH BARCODE) ---
class ProductVariant(models.Model):
    """Merepresentasikan Varian Barang SPESIFIK (e.g., Aspal Pertamina 60/70)"""
    base_item_code = models.ForeignKey(
        ItemCodeBarang,
    ...
    type_name = models.CharField(
        ...
        max_length=100,
        db_index=True,
        help_text="Contoh: Pulpen, Pensil, Sabun Cuci Tangan"
    )
    specific_code = models.CharField(
        _('kode spesifik'),
        max_length=3,
        editable=False,
        blank=True,
        help_text="3 digit unik per kode barang dasar, di-generate otomatis."
    )
    full_code = models.CharField(
        _('kode barang lengkap'),
        max_length=20,
        unique=True,
        editable=False,
        blank=True,
    ...
    name = models.CharField(
        _('nama spesifik (merk/tipe)'),
        max_length=150,
        help_text="Contoh: Snowman 0.5 Hitam, Faber-Castell 2B, Lifebuoy Mild Care"
    )
    description = models.TextField(
        _('deskripsi tambahan'), blank=True, null=True
    )
    unit_of_measure = models.CharField(
        _('satuan'),
        max_length=20,
        default='pcs',
        help_text="Contoh: pcs, box, rim, unit, kg, liter"
    )
    barcode = models.CharField(
        ...
        blank=True,
        db_index=True,
        help_text="Nilai barcode unik untuk varian ini, biasanya sama dengan Kode Lengkap."
    )
    ...
    def _generate_specific_code(self):
         ...
         if last_variant and last_variant.specific_code:
             try: next_num = int(last_variant.specific_code) + 1
                 ...
                 next_num = ProductVariant.objects.filter(
                     base_item_code=self.base_item_code
    ...
    def save(self, *args, **kwargs):
         ...
         if self.base_item_code_id and self.specific_code and (not self.pk or not self.full_code):
             try:
                 ...
                 raise IntegrityError(f"ItemCodeBarang ID {self.base_item_code_id} tidak ditemukan.")
             except Exception as e:
                 traceback.print_exc()
                 raise IntegrityError(f"Gagal men-generate full_code: {e}")

         if self.full_code and (not self.pk or not self.barcode):
             self.barcode = self.full_code

         super().save(*args, **kwargs)

# --- MODEL KUITANSI (Receipt) ---
class Receipt(models.Model):
    receipt_number = models.CharField(
        _('nomor kuitansi/PO'), max_length=100, unique=True
    )
    supplier_name = models.CharField(
        _('nama pemasok'), max_length=255, blank=True, null=True
    )
    receipt_date = models.DateField(_('tanggal kuitansi/pembelian'))
    uploaded_file = models.FileField(
        _('file unggahan'), upload_to='receipts/%Y/%m/', blank=True, null=True
    )
    uploaded_by = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        related_name='uploaded_receipts',
        on_delete=models.PROTECT,
        verbose_name=_('diunggah oleh')
    )
...
class InventoryItem(models.Model):
    ...
    added_by = models.ForeignKey(
        ...
        verbose_name=_('ditambahkan oleh')
    )
    receipt = models.ForeignKey(
        ...
        null=True, blank=True,
        on_delete=models.SET_NULL,
        verbose_name=_('kuitansi terkait')
    )
...
class Stock(models.Model):
    ...
    low_stock_threshold = models.PositiveIntegerField(
        _('ambang batas stok rendah'), default=10, blank=True, null=True
    )
...
class Request(models.Model):
    ...
    requester = models.ForeignKey(
        settings.AUTH_USER_MODEL, related_name='requests_made',
        on_delete=models.PROTECT, verbose_name=_('peminta')
    )
    request_number = models.CharField(
        _('nomor permintaan'), max_length=50, unique=True,
    ...
    submitted_at = models.DateTimeField(
        _('diajukan tanggal'), null=True, blank=True
    )
    supervisor1_approver = models.ForeignKey(
        settings.AUTH_USER_MODEL, related_name='approved_requests_spv1',
        null=True, blank=True, on_delete=models.SET_NULL,
        verbose_name=_('penyetuju (Atasan Peminta)')
    )
    supervisor1_decision_at = models.DateTimeField(
        _('tanggal keputusan spv1'), null=True, blank=True
    )
    supervisor1_rejection_reason = models.TextField(
        _('alasan penolakan spv1'), blank=True, null=True
    )
    supervisor2_approver = models.ForeignKey(
        settings.AUTH_USER_MODEL, related_name='approved_requests_spv2',
        null=True, blank=True, on_delete=models.SET_NULL,
        verbose_name=_('penyetuju (Atasan Operator)')
    )
    supervisor2_decision_at = models.DateTimeField(
        _('tanggal keputusan spv2'), null=True, blank=True
    )
    supervisor2_rejection_reason = models.TextField(
        _('alasan penolakan spv2'), blank=True, null=True
    )
    operator_processor = models.ForeignKey(
        settings.AUTH_USER_MODEL, related_name='processed_requests_opr',
        null=True, blank=True, on_delete=models.SET_NULL,
        verbose_name=_('pemroses (Operator)')
    )
    operator_processed_at = models.DateTimeField(
        _('tanggal diproses operator'), null=True, blank=True
    )
    operator_rejection_reason = models.TextField(
        _('alasan penolakan operator'), blank=True, null=True
    )
    received_at = models.DateTimeField(
        _('diterima peminta tanggal'), null=True, blank=True
    )
    ...
    def _generate_request_number(self):
        if not self.requester_id or not getattr(self.requester, 'department_code', None):
             return None
        year = timezone.now().year; department_code = self.requester.department_code
        prefix = f"ND-"; suffix = f"/{department_code}/PS/{year}"
        last_request = Request.objects.filter(
            request_number__startswith=prefix, request_number__endswith=suffix
        ).exclude(pk=self.pk).order_by('request_number').last()
        sequence = 1
...
class RequestItem(models.Model):
    request = models.ForeignKey(
        Request, related_name='items', on_delete=models.CASCADE,
        verbose_name=_('permintaan')
    )
    variant = models.ForeignKey(
        ProductVariant, related_name='requested_in', on_delete=models.PROTECT,
        verbose_name=_('varian produk')
    )
    quantity_requested = models.PositiveIntegerField(_('jumlah diminta'))
    quantity_approved_spv2 = models.PositiveIntegerField(
        _('jumlah disetujui Atasan Operator'), null=True, blank=True
    )
    quantity_issued = models.PositiveIntegerField(
        _('jumlah dikeluarkan'), default=0
    )
    ...
    def clean(self):
        if self.quantity_approved_spv2 is not None and self.quantity_approved_spv2 > self.quantity_requested:
            raise ValidationError(
                _('Jumlah disetujui tidak boleh melebihi jumlah yang diminta.')
            )

# --- MODEL SPMB ---
class SPMB(models.Model):
    request = models.OneToOneField(
        Request, related_name='spmb_document', on_delete=models.CASCADE,
        verbose_name=_('permintaan terkait')
    )
    spmb_number = models.CharField(
        _('nomor SPMB'), max_length=50, unique=True, editable=False
    )
    issued_by = models.ForeignKey(
        settings.AUTH_USER_MODEL, related_name='spmb_issued',
        on_delete=models.PROTECT, verbose_name=_('diterbitkan oleh (Operator)')
    )
    ...
    def _generate_spmb_number(self):
        year = timezone.now().year; wbc_code = "WBC.05" # HARDCODED
        prefix = f"SPMB-"; suffix = f"/{wbc_code}/PS/{year}"
        last_spmb = SPMB.objects.filter(
            spmb_number__startswith=prefix, spmb_number__endswith=suffix
...
class RequestLog(models.Model):
    request = models.ForeignKey(
        Request, related_name='logs', on_delete=models.CASCADE,
        verbose_name=_('permintaan')
    )
    user = models.ForeignKey(
        settings.AUTH_USER_MODEL, null=True, blank=True,
        on_delete=models.SET_NULL, verbose_name=_('pengguna')
    )
    ...
    status_from = models.CharField(
        _('status dari'), max_length=20, choices=Request.Status.choices,
        null=True, blank=True
    )
    status_to = models.CharField(
        _('status ke'), max_length=20, choices=Request.Status.choices,
        null=True, blank=True
    )
    ...
    def __str__(self):
        ...
        req_display = str(self.request) # Panggil __str__ dari Request
        return f"{self.timestamp.strftime('%Y-%m-%d %H:%M')} - {req_display} - {self.action} by {user_display}"
...
class Transaction(models.Model):
    ...
    variant = models.ForeignKey(
        ProductVariant, on_delete=models.PROTECT, verbose_name=_('varian produk')
    )
    inventory_item = models.ForeignKey(
        InventoryItem, null=True, blank=True, on_delete=models.SET_NULL,
        verbose_name=_('item inventaris terkait')
    )
    quantity = models.IntegerField(
        _('jumlah'), help_text="Positif untuk IN/ADJUST+, Negatif untuk OUT/ADJUST-"
    )
    transaction_type = models.CharField(
        _('tipe transaksi'), max_length=10, choices=Type.choices
    )
    ...
    related_spmb = models.ForeignKey(
        SPMB, null=True, blank=True, on_delete=models.SET_NULL,
        verbose_name=_('SPMB terkait')
    )
    receipt = models.ForeignKey(
...
class StockOpnameSession(models.Model):
    ...
    uploaded_file = models.FileField(
        _('file unggahan'), upload_to='stock_opname/%Y/%m/', blank=True, null=True
    )
    created_by = models.ForeignKey(
        settings.AUTH_USER_MODEL, related_name='opname_sessions_created',
        on_delete=models.PROTECT, verbose_name=_('dibuat oleh (Admin)')
    )
    created_at = models.DateTimeField(_('dibuat tanggal'), auto_now_add=True)
    status = models.CharField(
        _('status sesi'), max_length=20, choices=Status.choices,
...
class StockOpnameItem(models.Model):
    ...
    opname_session = models.ForeignKey(
        StockOpnameSession, related_name='items', on_delete=models.CASCADE,
        verbose_name=_('sesi opname')
    )
    variant = models.ForeignKey(
        ProductVariant, on_delete=models.PROTECT, verbose_name=_('varian produk')
    )
    ...
    confirmation_status = models.CharField(
        _('status konfirmasi'), max_length=10, choices=ConfirmationStatus.choices,
        default=ConfirmationStatus.PENDING
    )
    confirmed_by = models.ForeignKey(
        settings.AUTH_USER_MODEL, related_name='confirmed_opname_items',
        null=True, blank=True, on_delete=models.SET_NULL,
        verbose_name=_('dikonfirmasi oleh (Operator)')
    )
    confirmation_notes = models.TextField(
        _('catatan konfirmasi'), blank=True, null=True
    )
    confirmed_at = models.DateTimeField(
        _('dikonfirmasi tanggal'), null=True, blank=True
    )
